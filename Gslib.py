...
class Gslib:
    ...
    def Gslib_read(self, filename, order_z='no', overwrite_file='no', zmax=0):
        ...
        self.data = pandas.read_csv(filepath_or_buffer=self.filename, header=None, names=self.prop_names,
                                    skiprows=self.n_lines_head, sep='\s+')
    ...
    def Gslib_write(self, filename, prop_names, data, nx, ny, nz, folder, title='null'):
        ...
        if type(data) is numpy.ndarray:
            ...
            if self.n_properties == 1:
                if data.shape == (1, self.nx * self.ny * self.nz):
                    data = data.T  # if data is a 1D array,it is transposed for writing in a single column
                elif data.shape == (self.nx * self.ny * self.nz,):
                    pass  # If data is already in one column
                elif data.shape == (self.nx, self.ny, self.nz):
                    data = numpy.reshape(data, newshape=(nx * ny * nz, 1),
                                         order='F')  # if data is not a 1D array, but a 3D grid, changes shape to the array for writing
                numpy.savetxt(f, data, fmt='%.6f')

            elif self.n_properties > 1:

                if data[0].shape == (1, self.nx * self.ny * self.nz):
                    data = data.T  # if is an array of dimension= n_properties
                elif data[0].shape == (self.ny,) or data[0].shape == (self.n_properties,) or data[0].shape == (
                        self.nx * self.ny * self.nz, 1):
                    pass  # if data is an array of dimension= n_properties - for well logs
